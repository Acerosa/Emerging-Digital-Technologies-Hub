/**
 * ET per-hub Auth restore can lag behind Core's first learner.refresh().
...
export function createAuthGatedFetch(
  ...
  return async function authGatedFetch(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
    ...
    if (token) {
      ...
      console.info("L2E_AUTH_GATE_ATTACHED", {
    ...
    if (url.includes("ensure_learner_auth_link")) {
      console.info("L2E_AUTH_GATE_SKIP_ENSURE", { reason: "no-user-session" });
      return emptyJsonResponse();
    }

    console.info("L2E_AUTH_GATE_NO_SESSION", {
      rpc: url.split("/rest/v1/")[1] || url
    });
    return innerFetch(input, init);
  };
