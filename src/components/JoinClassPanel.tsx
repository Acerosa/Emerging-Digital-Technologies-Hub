...
function l2eEnrolment(enrolments: EnrolmentRow[] | null | undefined) {
  return (enrolments || []).find((row) =>
    String(row?.status || "").toLowerCase() === "active"
    && isAcceptedL2eGroupCode(row?.groupCode)
  ) || null;
}
...
export function JoinClassPanel({
  ...
  if (enrolled) {
    const joined = l2eEnrolment(enrolments);
    ...
    return (
      <section className="join-class panel" data-lp-join-class="enrolled" aria-labelledby="join-class-heading">
        ...
        {!compact ? (
          <p className="join-class__hint">You can check answers for assigned L2E activities.</p>
        ) : null}
      </section>
    );
  }
  ...
  return (
    ...
    >
      ...
      <p>
        Enter the registration key from your tutor to join your L2E class.
